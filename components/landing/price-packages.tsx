    ...
    features: [
      {
        ...
        explanation:
          "Personalize your bio link with a custom URL of your choice.",
      },
      {
        name: "Up to 10 Links",
        explanation:
      ...
      {
        name: "Profile Picture and Background Image",
        explanation:
          "Make your bio link stand out by adding your own profile picture and background image.",
      },
      {
        name: "Discord Presence",
        explanation:
          "Show your Discord status on your bio link and let your audience know when you're online.",
      },
    ],
  },
  {
        ...
        {prices.map((item, index) => {
          ...
          return (
            <CardSpotlightEffect
              key={index}
              className={cn(
                "self-end bg-white p-0 dark:bg-background",
                featured && "border-2 border-primary shadow-primary/25",
              )}
            >
              {featured && (
                <Badge className="absolute right-0 top-0 m-2">
                  Recommended
                </Badge>
              )}
              {featured && (
              ...
              <div className="p-6">
                <div className="mb-2 flex items-center gap-2">
                  <div
                    className={cn(
                      "size-4 rounded-full bg-foreground",
                      featured && "bg-primary",
                    )}
                  ></div>
                ...
                <div className="flex items-baseline gap-2">
                  <div className="text-freground text-3xl font-bold">
                    €{item.price}
                  </div>
              ...
              <div className="border-y bg-secondary p-6">
                <Link
                  href="/kontakt"
                  className={cn(
                    buttonVariants({
                      variant: featured ? "default" : "foreground",
                      size: "lg",
                    }),
                    "w-full rounded-full",
                  )}
                ...
                <ul className="mb-4 space-y-2">
                  {item.features.map((item, index) => (
                    <li key={index} className="flex items-center gap-4">
                      <svg
                        className={cn(
                          "size-2 shrink-0 fill-foreground",
                          featured && "fill-primary",
                        )}
                        viewBox="0 0 12 12"
...
const WavyImage = () => (
  <svg className="" width="158" height="119" aria-hidden="true">
    <defs>
